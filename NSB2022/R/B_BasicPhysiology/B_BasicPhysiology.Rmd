...
matlFileName = file.choose()
tmp <- readMat(matlFileName)

#tmp <- readMat("~/NotCloudy/NSB2022/MatlabManual/old/pUNITsv6.mat")
#tmp <- readMat("~/Desktop/pUNITsv6.mat")
```
...
# Load the EOD zero crossings from channel 2    
    EOD0Xings <- as.numeric(unlist(tmp$Ch2[5]))
...
ui <- basicPage(
  plotOutput("ThreshClick", click = "plot_click"),
  verbatimTextOutput("info"),
  actionButton("myBtn", "Close Window")
)

server <- function(input, output) {

  output$ThreshClick <- renderPlot({
    plot(tim[1:2000], vmfilt[1:2000], type="l")    
    })

  output$info <- renderText({
    thresh <<- input$plot_click$y
    paste0("Threshold= ", thresh, " mV.")
  })
...
# Convert to seconds
    ...
    meanSpikeRate = 1 / mean(diff(spiketimes, lag=1));
...
# Assemble into dataframe for plots
  ...
  spikedots <- array(thresh, c(length(spiketimes),1))
  plotSpikes <- data.frame(spiketimes, spikedots)
  
ggplot() +
  geom_line(data=plotPhys, colour = "Blue", show.legend = FALSE, aes(x = tim, y = vmfilt)) +
  geom_line(data=ploThresh, colour = "Red", show.legend = FALSE, aes(x = Xs, y = Ys)) +
  geom_point(data=plotSpikes, colour = "Black", show.legend = FALSE, aes(x = spiketimes, y = spikedots)) +
  xlab("Time, S") +
  ylab("Vm, mV") +
  ggtitle("Yeet") +
  xlim(0, windowidth)    
...
# Take data before the first stimulus starts (marker[1])
    ...
    disp('Mean baseline firing rate of the neuron is: ', meanBaseLinefiringrate, ' Hz.')
    isi <- diff(spiketimesBaseLine, lag=1);
...
# Definition of CV
    ...
    disp('Mean ISI (ms): ', mean_isi*1000, ' CV: ', CV)

```

### 2. Plot baseline interspike interval distribution (ISI Histogram)

```{r isiHist, warning=FALSE}

# Make a histogram of ISIs - we need bins over the range of ISI durations
    ISIrange = c(0.000, 0.010) # CHOOSE WISELY! This will vary between neurons. (in seconds)

    # Create 100 bins evenly distributed between the min and max of isi
    numbins = 100 # This is the number of bins (default 100)
    ISIbins <- seq(from=ISIrange[1], to=ISIrange[2], length=numbins)
    
    # Make the histogram
    numbercounts = histc(isi, ISIbins) # Calculate the histogram

# plot ISI histogram
    plotISIHist <- data.frame(ISIbins, numbercounts$cnt)
    
ggplot(data=plotISIHist) +
  geom_col(colour = "Blue", show.legend = FALSE, aes(x = ISIbins, y = numbercounts$cnt)) +
  xlab("ISI (ms)") +
  ylab("Counts") +
  ggtitle("Inter-Spike Interval Histogram") 
...
spiketimesBaseEOD = spiketimesBaseLine[which(spiketimesBaseLine > EOD0Xings[1] & spiketimesBaseLine < EOD0Xings[length(EOD0Xings)])]

# Created an empty vector length of the zerocrossings
      EODSpikephase = array(0, c(length(spiketimesBaseEOD), 1)) # PreAllocate for speed

# Loop over the spiketimes
    for (I in 1:length(spiketimesBaseEOD)) { # For each spike... 
        ind = which(EOD0Xings <= spiketimesBaseEOD[I]) # Get previous EOD zero Xing
        ind = tail(ind, n=1)
        
    # Phase is 2pi times the ratio [time from EOD crossing prior to spike] / [duration of the EOD crossing to the subsequent]     
...
# Plot phase distribution
    ...
    PhaseHistedges <- seq(from=0, to=2*pi, length=numbins)
    PhaseHistbins = histc(EODSpikephase, PhaseHistedges);
    
# plot EOD Phase histogram
    plotEODPhase <- data.frame(PhaseHistedges, PhaseHistbins$cnt)
    
ggplot(data=plotEODPhase) +
  geom_col(colour = "Blue", show.legend = FALSE, aes(x = PhaseHistedges, y = PhaseHistbins$cnt)) +
  xlab("Radians - 2pi") +
  ylab("Counts") +
  ggtitle("EOD Phase Histogram") 

```

### 4. Measure degree of phase locking (vector strength) to EOD

Vector strength is used as a measure of phase locking in the auditory system and ranges between zero and one.

```{r EODVectorStrength, warning=TRUE}

# Compute the vector strength (VS)
    vector_strength = sqrt(mean(cos(EODSpikephase))^2 + mean(sin(EODSpikephase))^2);

    disp('VS: ', vector_strength)
