...
Once we have R studio running we can begin learning. To complete these tutorials you'll need several packages: seeWave, TuneR, ggplot2, and gridExtra. TuneR allows us to read .wav files, seeWave allows us to plot easy spectrograms, and ggplot2 and gridExtra allow us to plot other graphs. We will also be using the built in graphing functions for certain plots (so you'll learn to use both ggplot and built in R graphics)
...
ggplot2 by default uses dataframes to plot. We don't need to use them here, but its a good idea to get used to using R dataframes eventually. 

We create initiate a plot window using **ggplot()** and we can add to that plot by using a **+**

```{r}
ggplot() +
  tim, wav, xlab = 'Time', ylab = 'Sine Wave', col = "Red", type = "l")
