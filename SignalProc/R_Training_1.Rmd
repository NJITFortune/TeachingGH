...
We will install these packages now. In the future you may choose to use different packages or even make your own. For now lets just use these; copy and paste into a blank R script to run. You should only have to install these packages once and if you use the same R workspace, you shouldn't have to reload libraries.  

```{r, include=FALSE}
install.packages("signal") #for producing specgram
install.packages("tuneR") #for reading .wav files
install.packages("ggplot2") # for plotting 
install.packages("oce") #plot images 
install.packages("seewave") #handling audio data
install.packages("gridExtra") #for formatting ggplot graphs
devtools::install_github("NJITFortune/DFWE") #custom package for plotting spectrogram
```
